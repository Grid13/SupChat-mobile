...
const CreateWorkspaceModal: React.FC<Props> = ({ visible, onClose, onCreated }) => {
  ...
  const fetchAvailableWorkspaces = async () => {
    ...
    try {
      const response = await fetch("http://localhost:5263/api/Workspace/Available", {
          ...
          onPress: async () => {
            try {
              ...
              const res = await fetch(
                `http://localhost:5263/api/Workspace/${workspace.id}/Join`,
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const res = await fetch(
        `http://localhost:5263/api/Attachment?attachmentType=ProfilePicture`,
        {
          method: 'POST',
  ...
  const createWorkspace = async () => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:5263/api/Workspace", {
        method: "POST",
        headers: {
          Accept: "text/plain",
